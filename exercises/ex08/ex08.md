...
GLib is a collection of predefined advanced data structures for general use by programmers who want to
use complicated data structures without handling definition of the structures or the basic functions 
commonly used in association with the structures. It probably would have been clearer to distinguish
between the two better because GLib is the library fundamental to the GTK+ and GNOME projects and glibc refers
to the GNU library of key C functions.

*  What are the most important pros and cons of using GLib, as opposed
to another library, or just using core C features?

GLib is an extensive collection of useful data structures that can be applied easily to GUI related 
applications. However, other libraries are better at portability related applications than GLib.

*  What do you have to do to compile and run a "Hello GLib" example?

In order to run a "Hello GLib" example, you need to download GLib, use the pkg-config utility to 
tell the compiler where to find the library relevant files, and include the relevant headers 
