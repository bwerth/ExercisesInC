...
void freeyer(gpointer key, gpointer value, gpointer user_data){
    g_free(value);
    g_free(user_data);
}

void freeseq(gpointer value, gpointer user_data){
    Pair *pair = (Pair *) value;
    g_free(pair);
    g_free(user_data);
}
...
int main(int argc, char** argv)
{
    ...
    g_hash_table_foreach(hash, (GHFunc) freeyer, NULL);
    g_sequence_foreach(seq, (GFunc) freeseq, NULL);
